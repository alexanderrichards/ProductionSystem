"""
SQLAlchemy Base Table Module.
"""
from __future__ import annotations

import json
from abc import ABCMeta
from collections.abc import Iterable
from datetime import datetime
from enum import Enum

from sqlalchemy import Column
from sqlalchemy.orm import DeclarativeBase
from sqlalchemy.orm.attributes import InstrumentedAttribute
from sqlalchemy.orm.exc import DetachedInstanceError

__all__ = ('SQLTableBase', )


class DeclarativeABCMeta(type(DeclarativeBase), ABCMeta):
    """
    Combine SQLAlchemy 2 declarative attributes with abstract bases.
    """
class SmartColumn(Column):
    """
    A Column carrying extra Python-side required/allowed metadata.
    """
    # SmartColumn adds no SQL behaviour over Column, only Python-side metadata,
    # so the cache key generated by the superclass remains valid.
    inherit_cache = True

    def __init__(self, *args, **kwargs):
        """
        Initialize the column and record Python-side validation metadata.
        
        Args:
            *args: Positional arguments forwarded to ``Column``.
            **kwargs: Column options plus optional ``required`` and ``allowed`` flags.
        """
        self._required = kwargs.pop('required', False)
        self._allowed = self._required or kwargs.pop('allowed', False)
        super().__init__(*args, **kwargs)

    @property
    def required(self):
        """
        Return whether this column is required by the application schema.
        
        Returns:
            bool: True when the column is marked required.
        """
        return self._required

    @property
    def allowed(self):
        """
        Return whether this column may be supplied by clients.
        
        Returns:
            bool: True when the column may be provided by clients.
        """
        return self._allowed


class ColumnsDescriptor(object):
    """
    Yield the column names.
    """
    def __init__(self, required=False, allowed=False):
        """
        Initialise.

        Args:
            required: If True, yield only columns marked required.
            allowed: If True, yield only columns marked client-allowed.
        """
        self._required = required
        self._allowed = allowed

    def __get__(self, obj, cls):
        """
        Descriptor get.

        Args:
            obj: Instance access target; unused because column names come from the class.
        """
        for column in cls.__table__.columns:
            # use getattr so works (doesnt break) on normal column as well as smart column
            if self._required and not getattr(column, 'required', False):
                continue
            if self._allowed and not getattr(column, 'allowed', False):
                continue
            yield column.name

    def __set__(self, obj, value):
        """
        Descriptor set.

        Args:
            obj: Instance on which assignment was attempted.
            value: Ignored assignment value because the descriptor is read-only.
        """
        raise AttributeError("Read only attribute!")


class IterableBase(Iterable):
    """
    Iterable base class.

    A base class that provides the functionality of
    being able to iterate over the instrumented attributes
    of an SQLAlchemy declarative base.
    """

    # This we can get from the class as well as instance
    # unlike property
    columns = ColumnsDescriptor()
    required_columns = ColumnsDescriptor(required=True)
    allowed_columns = ColumnsDescriptor(allowed=True)

    def __iter__(self):
        """
        Get an iterator over instrumented attributes.
        """
        for name, type_ in vars(self.__class__).items():
            if isinstance(type_, (InstrumentedAttribute, property)):
                try:
                    getattr(self, name, None)
                except DetachedInstanceError:  # This is lazy loaded and not available
                    continue
                yield name

    def __getitem__(self, item):
        """
        Access instrumented attributes as a dict.

        Args:
            item: Instrumented attribute name to read.

        Returns:
            object: Attribute value for the requested SQLAlchemy field.
        """
        if not hasattr(self, item):
            raise KeyError("Invalid attribute name: %s" % item)
        return getattr(self, item, None)  # None should never be needed here try deleting

    def __len__(self):
        """
        Return the number of available instrumented attributes.
        
        Returns:
            int: Count of currently available instrumented attributes.
        """
        return len(list(iter(self)))

    def __hash__(self):
        """
        Return an identity-based hash for the table instance.
        
        Returns:
            int: Identity-based hash of the table instance.
        """
        return id(self)

    def __eq__(self, other):
        """
        Compare table instances by object identity.
        
        Args:
            other: Object to compare with this table instance.

        Returns:
            bool: True when both references point to the same object identity.
        """
        return id(self) == id(other)

    @staticmethod
    def _jsonable_value(value):
        """
        Convert a single attribute value into a JSON encodable object.

        Returns:
            object: JSON-encodable representation of the supplied value.
        """
        # Enum must be checked before the scalar types as IntEnum/str Enum are
        # also instances of int/str and would otherwise serialise as their value.
        if isinstance(value, Enum):
            return value.name.capitalize()
        if isinstance(value, datetime):
            return value.isoformat(' ')
        if isinstance(value, IterableBase):
            return value.jsonable_dict()
        if isinstance(value, (str, int, float, bool, type(None))):
            return value
        if isinstance(value, (list, tuple, set, frozenset)):
            return [IterableBase._jsonable_value(item) for item in value]
        if isinstance(value, dict):
            return {key: IterableBase._jsonable_value(item) for key, item in value.items()}
        return str(value)

    def jsonable_dict(self):
        """
        Return an easily JSON encodable object.

        Returns:
            dict: Mapping of iterable table attributes to JSON-encodable values.
        """
        return {column: self._jsonable_value(self[column]) for column in self}

    def to_json(self):
        """
        Return a JSON representation of the object.

        Returns:
            str: JSON string representation of this table instance.
        """
        return json.dumps(self.jsonable_dict())


class SQLTableBase(IterableBase, DeclarativeBase, metaclass=DeclarativeABCMeta):
    """
    Modern SQLAlchemy 2.0+ declarative base.

    Combines DeclarativeBase with IterableBase for enhanced functionality.
    """
    __abstract__ = True
